...
public class PersonData : IEntityTypeConfiguration<Person>
{
    ...
    {
        builder.HasOne(p => p.Photo).WithOne(ph => ph.Person).HasForeignKey<Person>(p => p.PersonId);
        builder.HasMany(p => p.Orders).WithOne(p => p.Person).HasForeignKey(o => o.PersonId);
